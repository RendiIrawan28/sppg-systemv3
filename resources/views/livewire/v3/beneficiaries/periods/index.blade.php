    ...
    <div class="mx-auto max-w-[1400px] space-y-5">
        ...
        <section class="relative overflow-hidden rounded-[26px] bg-[#081d3a] p-6 text-white shadow-xl sm:p-7">
            ...
            <div class="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
                <div>
                    <span class="inline-flex rounded-full bg-cyan-300/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.16em] text-cyan-200 ring-1 ring-cyan-300/20">Dua pilihan sumber data</span>
                    <h2 class="mt-4 text-2xl font-bold tracking-tight sm:text-3xl">Periode penerima setiap 14 hari.</h2>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Gunakan snapshot penerima aktif berdasarkan nama, atau input jumlah manual per sekolah/Posyandu dan kategori.</p>
                </div>
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[900px] text-left">
                    <thead><tr class="border-b border-slate-100 bg-slate-50/70 text-[10px] font-bold uppercase tracking-wider text-slate-400"><th class="px-5 py-3.5">Periode</th><th class="px-5 py-3.5">Rentang</th><th class="px-5 py-3.5">Sumber</th><th class="px-5 py-3.5 text-center">Instansi</th><th class="px-5 py-3.5 text-center">Penerima aktif</th><th class="px-5 py-3.5">Status</th><th class="px-5 py-3.5">Nomor dokumen</th><th class="px-5 py-3.5 text-right">Aksi</th></tr></thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($periods as $period)
                            ...
                            <tr class="hover:bg-sky-50/30"><td class="px-5 py-4"><p class="text-sm font-bold text-slate-800">{{ $period->name }}</p><p class="mt-0.5 text-xs text-slate-400">{{ $period->code }}</p></td><td class="px-5 py-4 text-sm text-slate-600">{{ $period->start_date->translatedFormat('d M') }}–{{ $period->end_date->translatedFormat('d M Y') }}</td><td class="px-5 py-4"><span class="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600">{{ $period->category_totals_exists ? 'Manual' : 'Snapshot nama' }}</span></td><td class="px-5 py-4 text-center text-sm font-bold text-slate-700">{{ $period->destination_count }}</td><td class="px-5 py-4 text-center text-sm font-bold text-slate-700">{{ number_format($period->active_members, 0, ',', '.') }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold ring-1 {{ $statusClass }}">{{ $period->statusLabel() }}</span></td><td class="px-5 py-4 text-xs text-slate-500">{{ $period->document_number ?: '—' }}</td><td class="px-5 py-4"><div class="flex justify-end gap-2">@if ($period->isEditable() && (auth()->user()->is_super_admin || auth()->user()->can('beneficiary_periods.update')))<a wire:navigate href="{{ route('v3.beneficiary-periods.edit', $period) }}" class="grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-600" aria-label="Ubah draft"><x-v3.icon name="pencil" class="size-4" /></a>@endif<a wire:navigate href="{{ route('v3.beneficiary-periods.show', $period) }}" class="rounded-lg bg-sky-50 px-3 py-2 text-[10px] font-bold text-sky-700 ring-1 ring-sky-100">Ringkasan</a></div></td></tr>
                        @empty
                            <tr><td colspan="8" class="px-5 py-16 text-center"><span class="mx-auto grid size-12 place-items-center rounded-2xl bg-slate-100 text-slate-400"><x-v3.icon name="calendar" class="size-5" /></span><p class="mt-4 text-sm font-bold text-slate-700">Periode penerima belum tersedia</p><p class="mt-1 text-xs text-slate-400">Buat periode dari snapshot master atau input jumlah manual.</p></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
