<x-v3.shell :$unit :$navigation :$roleLabel title="Rincian Penerimaan" eyebrow="Kuantitas, batch, dan QC">
    <div class="mx-auto max-w-[1450px] space-y-5">
        ...
        <section class="rounded-[26px] bg-[#081d3a] p-6 text-white shadow-xl sm:p-7"><div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><span class="rounded-full bg-emerald-300/10 px-3 py-1 text-[10px] font-bold text-emerald-200 ring-1 ring-emerald-300/20">{{ $statuses[$receipt->status] ?? '-' }}</span><p class="mt-4 text-xs font-bold uppercase tracking-[.14em] text-cyan-200">Kiriman {{ $receipt->supplier?->name ?? 'Supplier' }}</p><h3 class="mt-1 text-2xl font-bold">{{ $receipt->items->count() }} item diterima bersama</h3><p class="mt-2 text-sm text-slate-300">{{ $receipt->notes ?: 'Catat jumlah barang dan unggah satu foto untuk keseluruhan kiriman supplier.' }}</p></div><div class="grid grid-cols-3 gap-3"><div class="rounded-xl border border-white/10 bg-white/[.07] p-3"><p class="text-[10px] text-slate-400">Dipesan</p><p class="mt-1 text-lg font-bold">{{ number_format((float) $receipt->items->sum('ordered_quantity_kg'), 2, ',', '.') }} kg</p></div><div class="rounded-xl border border-white/10 bg-white/[.07] p-3"><p class="text-[10px] text-slate-400">Diterima baik</p><p class="mt-1 text-lg font-bold">{{ number_format((float) $receipt->items->sum('accepted_quantity_kg'), 2, ',', '.') }} kg</p></div><div class="rounded-xl border border-rose-300/20 bg-rose-300/10 p-3"><p class="text-[10px] text-rose-200/70">Ditolak</p><p class="mt-1 text-lg font-bold text-rose-100">{{ number_format((float) $receipt->items->sum('rejected_quantity_kg'), 2, ',', '.') }} kg</p></div></div></div></section>
        ...
        <section class="space-y-4">
            @foreach ($receipt->items as $item)
                <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div class="flex flex-col justify-between gap-2 border-b border-slate-100 pb-4 sm:flex-row sm:items-center"><div><h3 class="text-base font-bold text-slate-950">{{ $item->ingredient_name_snapshot }}</h3><p class="mt-1 text-xs text-slate-400">Dipesan {{ number_format((float) $item->ordered_quantity, 4, ',', '.') }} {{ $item->unit_snapshot }} / {{ number_format((float) $item->ordered_quantity_kg, 3, ',', '.') }} kg · {{ $item->supplier?->name ?? 'Supplier tidak tersedia' }}</p></div><span class="w-fit rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-600 ring-1 ring-slate-200">{{ ['pending' => 'Belum dicek', 'accepted' => 'Layak', 'partial' => 'Sebagian ditolak', 'rejected' => 'Ditolak'][$item->quality_status] ?? '-' }}</span></div>
