    ...
    <div class="mx-auto max-w-[1200px] space-y-4">
        <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
                ...
                <div class="mt-4 inline-flex rounded-xl bg-slate-100 p-1"><button wire:click="$set('warehouseType','food')" type="button" class="rounded-lg px-4 py-2 text-xs font-bold {{ $warehouseType === 'food' ? 'bg-[#081d3a] text-white' : 'text-slate-600' }}">Pangan</button><button wire:click="$set('warehouseType','non_food')" type="button" class="rounded-lg px-4 py-2 text-xs font-bold {{ $warehouseType === 'non_food' ? 'bg-[#081d3a] text-white' : 'text-slate-600' }}">Non-Pangan</button></div>
            </div>
                    ...
                    @foreach($rows as $index => $row)
                        <article wire:key="opening-stock-row-{{ $index }}" class="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                            ...
                            <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-12">
                                <label class="md:col-span-3"><span class="mb-1 block text-xs font-semibold text-slate-600">Sumber barang</span><select wire:model.live="rows.{{ $index }}.mode" class="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"><option value="existing">{{ $warehouseType === 'non_food' ? 'Master Non-Pangan' : 'Master Bahan' }}</option><option value="new">Barang baru</option></select></label>
                                @if($row['mode'] === 'existing')
                                    @php($itemField = $warehouseType === 'non_food' ? 'non_food_item_id' : 'ingredient_id')
                                    <div x-data="{ open: false, search: @js(collect($ingredientSearchOptions)->firstWhere('id', (string) ($row[$itemField] ?? ''))['label'] ?? '') }" x-on:click.outside="open = false" class="relative md:col-span-7"><label><span class="mb-1 block text-xs font-semibold text-slate-600">Cari barang *</span><input x-model="search" x-on:focus="open = true" x-on:input="open = true; $wire.set('rows.{{ $index }}.{{ $itemField }}', '')" type="search" autocomplete="off" class="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm" placeholder="Ketik nama, kode, atau satuan barang"></label><div x-show="open" x-transition class="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl"><template x-for="option in ingredientOptions.filter(item => item.label.toLowerCase().includes(search.toLowerCase().trim())).slice(0, 50)" :key="option.id"><button type="button" x-on:click="search = option.label; open = false; $wire.set('rows.{{ $index }}.{{ $itemField }}', option.id)" class="block w-full rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-sky-50" x-text="option.label"></button></template><p x-show="ingredientOptions.filter(item => item.label.toLowerCase().includes(search.toLowerCase().trim())).length === 0" class="px-3 py-4 text-center text-xs text-slate-400">Barang tidak ditemukan</p></div>@error("rows.$index.$itemField")<span class="mt-1 block text-xs text-rose-600">{{ $message }}</span>@enderror</div>
                                    <label class="md:col-span-2"><span class="mb-1 block text-xs font-semibold text-slate-600">Satuan</span><input value="{{ $ingredientUnits[$row[$itemField] ?? ''] ?? '—' }}" disabled class="h-10 w-full rounded-xl border border-slate-200 bg-slate-100 px-3 text-sm font-semibold text-slate-500"></label>
                                @else
                                ...
                                @endif
                                @if($warehouseType === 'non_food' && $row['mode'] === 'new')
                                    <label class="md:col-span-2"><span class="mb-1 block text-xs font-semibold text-slate-600">Stok minimum</span><input wire:model="rows.{{ $index }}.minimum_stock" type="number" min="0" step="0.0001" class="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"></label>
                                    <label class="md:col-span-2"><span class="mb-1 block text-xs font-semibold text-slate-600">Target stok</span><input wire:model="rows.{{ $index }}.target_stock" type="number" min="0" step="0.0001" class="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"></label>
                                    <label class="flex items-center gap-2 md:col-span-2"><input wire:model="rows.{{ $index }}.tracks_lot" type="checkbox" class="rounded border-slate-300"><span class="text-xs font-semibold text-slate-600">Wajib lot</span></label>
                                    <label class="flex items-center gap-2 md:col-span-2"><input wire:model="rows.{{ $index }}.tracks_expiry" type="checkbox" class="rounded border-slate-300"><span class="text-xs font-semibold text-slate-600">Wajib kedaluwarsa</span></label>
                                @endif
