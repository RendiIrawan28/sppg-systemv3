                            ...
                            @foreach ($relations['wasteRecords'] ?? [] as $index => $row)
                                <div wire:key="washing-waste-{{ $row['_id'] ?? 'new-'.$index }}" class="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 dark:border-amber-400/25 dark:bg-amber-500/5">
                                    ...
                                    <div class="mt-3 grid gap-3 sm:grid-cols-2">
                                        ...
                                        <label class="lg:col-span-2"><span class="mb-1 block text-xs font-semibold">Foto limbah</span><input wire:model="uploads.wasteRecords.{{ $index }}.photo_path" type="file" accept="image/*" @disabled(!$isReceived || !$canUpdate) class="block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs dark:border-slate-700 dark:bg-slate-950">@if (!empty($row['photo_path']))<a target="_blank" href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($row['photo_path']) }}" class="mt-2 inline-block text-xs font-bold text-sky-700 dark:text-sky-300">Lihat foto tersimpan</a>@endif</label>
                        ...
                        @foreach ($relations['documentations'] ?? [] as $index => $row)
                            <div wire:key="washing-photo-{{ $row['_id'] ?? 'new-'.$index }}" class="grid gap-3 rounded-xl border border-slate-200 p-4 dark:border-slate-700 sm:grid-cols-[1fr_auto]">
                                <div>
                                    ...
                                    @if (!empty($row['photo_path']))<a target="_blank" href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($row['photo_path']) }}" class="mt-2 inline-block text-xs font-bold text-sky-700 dark:text-sky-300">Lihat foto tersimpan</a>@endif
                                </div>
