<x-v3.shell :$unit :$navigation :$roleLabel title="Divisi Pemorsian" eyebrow="Pencatatan manual per rute">
    <div class="mx-auto max-w-[1500px] space-y-5">
        @if(session('v3.status'))
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm font-bold text-emerald-700">{{ session('v3.status') }}</div>
        @endif
        @if($errors->any())
            <div class="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-700">{{ $errors->first() }}</div>
        @endif
