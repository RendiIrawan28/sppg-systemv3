<x-v3.shell :$unit :$navigation :$roleLabel title="Presensi Pegawai" eyebrow="RFID masuk dan pulang">
    <div wire:poll.15s class="mx-auto max-w-[1450px] space-y-5">
        ...
        @if($activeTab === 'attendance')
                ...
                @forelse($sessionGroups as $group)
                    <div class="border-b border-slate-200 dark:border-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">@foreach($group['sessions'] as $session)<tr wire:key="attendance-{{ $session->id }}">
                            ...
                            @if($canCorrect || $canReset)<td class="p-3"><div class="flex gap-2">@if($canCorrect)<button wire:click="editSession({{ $session->id }})" type="button" class="rounded-lg bg-sky-50 px-3 py-2 text-xs font-bold text-sky-700 dark:bg-sky-950 dark:text-sky-200">Koreksi</button>@endif @if($canReset)<button wire:click="openDeleteSession({{ $session->id }})" type="button" class="rounded-lg bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 dark:bg-rose-950 dark:text-rose-200">Hapus</button>@endif</div></td>@endif
            ...
            @if($deleteSessionId && $canReset)
                <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" role="presentation">
                <section role="dialog" aria-modal="true" aria-labelledby="delete-attendance-title" class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-rose-200 bg-rose-50 p-5 shadow-2xl sm:p-6">
                    @if($deleteTarget)
                        <h3 id="delete-attendance-title" class="font-bold text-rose-950">Hapus satu data presensi</h3>
                        <p class="mt-2 text-sm text-rose-900">{{ $deleteTarget->user?->name ?? 'Pegawai tidak tersedia' }} · {{ $deleteTarget->work_date->format('d/m/Y') }} · Masuk {{ $deleteTarget->check_in_at?->format('H:i') ?: '—' }} · Pulang {{ $deleteTarget->check_out_at?->format('H:i') ?: '—' }} <span class="text-xs">(Data #{{ $deleteTarget->id }})</span></p>
                        <p class="mt-2 text-sm text-rose-800">Gunakan <strong>Koreksi</strong> jika hanya jam atau status yang salah. Hapus digunakan untuk data yang tidak sah atau ganda; catatan ini akan hilang dari laporan, tetapi riwayat tap RFID dan jejak penghapusan tetap tersimpan.</p>
                        <form wire:submit="deleteAttendanceSession" class="mt-5 space-y-4">
                            <div class="grid gap-4 sm:grid-cols-2">
                                <label><span class="mb-1 block text-xs font-semibold text-rose-900">Alasan penghapusan *</span><textarea wire:model="deleteReason" rows="3" class="w-full rounded-xl border border-rose-200 bg-white px-3 py-2 text-sm" placeholder="Contoh: presensi ganda akibat input manual"></textarea>@error('deleteReason')<span class="mt-1 block text-xs text-rose-700">{{ $message }}</span>@enderror</label>
                                <label><span class="mb-1 block text-xs font-semibold text-rose-900">Ketik HAPUS untuk konfirmasi *</span><input wire:model="deleteConfirmation" type="text" autocomplete="off" class="h-11 w-full rounded-xl border border-rose-200 bg-white px-3 text-sm" placeholder="HAPUS">@error('deleteConfirmation')<span class="mt-1 block text-xs text-rose-700">{{ $message }}</span>@enderror</label>
                            </div>
                            @error('deleteSessionId')<p role="alert" class="text-sm text-rose-700">{{ $message }}</p>@enderror
                            <div class="flex flex-wrap justify-end gap-2"><button wire:click="cancelDeleteSession" type="button" class="h-11 rounded-xl border border-rose-200 bg-white px-5 text-sm font-bold text-slate-700">Batal</button><button wire:loading.attr="disabled" wire:target="deleteAttendanceSession" type="submit" class="h-11 rounded-xl bg-rose-600 px-5 text-sm font-bold text-white disabled:opacity-50">Hapus presensi ini</button></div>
                        </form>
                    @else
                        <h3 id="delete-attendance-title" class="font-bold text-rose-950">Data presensi tidak tersedia</h3>
                        <p class="mt-2 text-sm text-rose-800">Data presensi yang dipilih sudah tidak tersedia pada tanggal ini.</p>
                        <button wire:click="cancelDeleteSession" type="button" class="mt-3 rounded-xl border border-rose-200 bg-white px-4 py-2 text-sm font-bold text-slate-700">Tutup</button>
                    @endif
                </section>
                </div>
            @endif
