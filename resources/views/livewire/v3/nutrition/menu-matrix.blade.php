    ...
    <div class="mx-auto max-w-[1600px] space-y-5">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-end">
                ...
                @if ($cycle)<div class="flex flex-wrap gap-2"><span class="inline-flex h-11 items-center rounded-xl bg-slate-100 px-3 text-xs font-bold text-slate-600">{{ $cycle->status->label() }}</span>@if (auth()->user()->is_super_admin || auth()->user()->can('menus.export'))<a href="{{ route('nutrition.menu-cycles.pdf', $cycle) }}" target="_blank" class="inline-flex h-11 items-center rounded-xl border border-slate-200 px-3 text-xs font-bold text-slate-600">PDF</a>@endif @if ($cycle->isEditable() && (auth()->user()->is_super_admin || auth()->user()->can('menus.update')))<button wire:click="saveAll" class="h-11 rounded-xl bg-emerald-600 px-4 text-xs font-bold text-white">Simpan semua</button>@endif</div>@endif
            </div>
        </section>
                ...
                <article wire:key="matrix-day-{{ $dayId }}" class="rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">@if ($row['can_edit'])
                        ...
                        <button wire:click="detachMenu({{ $dayId }})" wire:confirm="Lepas menu dari hari ini?" class="rounded-lg px-3 py-2 text-[10px] font-bold text-rose-700 hover:bg-rose-50">
                            Lepas
                        </button>@endif @if (isset($menuEditorUrls[$row['menu_id']]))
                        <a
                            wire:navigate
                            href="{{ $menuEditorUrls[$row['menu_id']] }}"
                            class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-bold text-slate-600">
                            Editor resep
                            <x-v3.icon name="pencil" class="ml-1 inline size-3" />
                        </a>
                        @endif
                        @endif
                    </div>
        ...
        <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_420px]">
            ...
            <aside class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-3 grid gap-2">@if ($cycle->isEditable() && (auth()->user()->is_super_admin || auth()->user()->can('menus.submit')))<button wire:click="submitCycle" wire:confirm="Validasi dan ajukan siklus ini?" class="h-10 rounded-xl bg-[#081d3a] text-xs font-bold text-white">Validasi & ajukan</button>@endif @if ($cycle->status === \App\Enums\NutritionRecordStatus::Submitted && (auth()->user()->is_super_admin || auth()->user()->can('menus.approve')))<button wire:click="approveCycle" wire:confirm="Setujui dan kunci siklus ini?" class="h-10 rounded-xl bg-emerald-600 text-xs font-bold text-white">Setujui siklus</button><button wire:click="returnCycle" class="h-10 rounded-xl bg-rose-50 text-xs font-bold text-rose-700 ring-1 ring-rose-100">Kembalikan untuk revisi</button>@endif @if ($cycle->status === \App\Enums\NutritionRecordStatus::Approved && (auth()->user()->is_super_admin || auth()->user()->can('menus.activate')))<button wire:click="activateCycle" wire:confirm="Aktifkan siklus ini? Siklus aktif lama akan diarsipkan." class="h-10 rounded-xl bg-emerald-600 text-xs font-bold text-white">Aktifkan & terapkan</button>@endif</div>
            </aside>
        </div>
        @else
