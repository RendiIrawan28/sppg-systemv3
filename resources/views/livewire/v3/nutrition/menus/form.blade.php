<x-v3.shell :$unit :$navigation :$roleLabel title="Editor Resep & Gramasi" eyebrow="Seperti lembar resep Ahli Gizi">
    <div class="mx-auto max-w-[1450px] space-y-5">
                            ...
                            <div class="mt-3 space-y-3">
                                @foreach ($item['ingredients'] as $ingredientIndex => $ingredient)
                                    <div class="grid gap-3 rounded-xl border {{ ($ingredient['portion_source'] ?? 'manual') === 'standard' ? 'border-emerald-200 bg-emerald-50/40' : 'border-amber-200 bg-amber-50/40' }} p-4 sm:grid-cols-2 xl:grid-cols-8">
                                        ...
                                        @if(($ingredient['ingredient_id'] ?? null) && (auth()->user()->is_super_admin || auth()->user()->can('nutrition.view')))<div class="flex items-end xl:col-span-2"><button type="button" wire:click="showIngredientNutrition({{ $itemIndex }}, {{ $ingredientIndex }})" class="h-9 w-full rounded-lg border border-sky-300 bg-sky-50 px-3 text-xs font-bold text-sky-800 hover:bg-sky-100 dark:border-sky-500/40 dark:bg-sky-500/10 dark:text-sky-200">Lihat Nilai Gizi</button></div>@endif
        ...
        @endif
        @include('livewire.v3.nutrition.partials.ingredient-nutrition-modal', ['detail' => $nutritionPreview, 'closeAction' => 'closeIngredientNutrition'])
    </div>
</x-v3.shell>
