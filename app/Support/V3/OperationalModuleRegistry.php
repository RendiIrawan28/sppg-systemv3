...
final class OperationalModuleRegistry
{
    ...
    public function definitions(): array
    {
        return [
            ...
            'kebersihan' => [
                'label' => 'Kebersihan',
                'description' => 'Checklist harian sesuai template resmi Toilet, Produksi, Pemorsian, dan Gudang.',
                'model' => CleaningSession::class,
                'permission' => 'cleaning',
                'number' => 'session_number',
                'date' => 'scheduled_date',
                'pdf' => 'cleaning-sessions.pdf',
                'fields' => [
                    $this->field('scheduled_date', 'Tanggal jadwal', 'date', true),
                    $this->field('shift', 'Shift', 'select', true, ['morning' => 'Pagi', 'afternoon' => 'Siang', 'night' => 'Malam']),
                    $this->field('scheduled_start_at', 'Rencana mulai', 'datetime'),
                    $this->field('cleaning_area_id', 'Area', 'select', true, 'cleaning_areas'),
                    $this->field('petugas_id', 'Petugas', 'select', false, 'users'),
                    $this->field('before_condition', 'Kondisi awal', 'textarea'),
                    $this->field('after_condition', 'Evaluasi kebersihan', 'textarea'),
                    $this->field('waste_presence', 'Limbah', 'select', false, ['none' => 'Tidak ada limbah', 'yes' => 'Ada limbah']),
                    $this->field('notes', 'Catatan pekerjaan', 'textarea'),
                ],
                'relations' => [
                    'checklistItems' => $this->relation('Checklist area', [
                        $this->field('category', 'Kategori', 'text'),
                        $this->field('item_name', 'Item kebersihan', 'text', true),
                        $this->field('is_mandatory', 'Wajib', 'boolean'),
                        $this->field('result', 'Hasil', 'select', true, [
                            'pending' => 'Belum diperiksa',
                            'pass' => 'Terpenuhi',
                            'fail' => 'Tidak terpenuhi',
                        ]),
                        $this->field('checked_at', 'Waktu pemeriksaan', 'datetime'),
                        $this->field('checked_by', 'Diperiksa oleh', 'select', false, 'users'),
                        $this->field('notes', 'Evaluasi item', 'textarea'),
                        $this->field('sort_order', 'Urutan', 'number'),
                    ]),
                    'chemicalUsages' => $this->relation('Bahan pembersih (opsional)', [
                        $this->field('chemical_name', 'Nama bahan', 'text', true),
                        $this->field('quantity', 'Jumlah', 'number', true),
                        $this->field('unit', 'Satuan', 'text', true),
                        $this->field('purpose', 'Kegunaan'),
                        $this->field('used_at', 'Waktu penggunaan', 'datetime'),
                        $this->field('notes', 'Catatan', 'textarea'),
                    ]),
                    'documentations' => $this->relation('Dokumentasi', [
                        $this->field('phase', 'Tahap', 'select', true, ['before' => 'Sebelum (opsional)', 'after' => 'Hasil kebersihan']),
                        $this->field('photo_path', 'Foto', 'file', true),
                        $this->field('caption', 'Keterangan'),
                        $this->field('captured_at', 'Waktu foto', 'datetime'),
                        $this->field('sort_order', 'Urutan', 'number'),
                    ]),
                    'findings' => $this->relation('Temuan (opsional)', [
                        $this->field('found_at', 'Waktu temuan', 'datetime', true),
                        $this->field('category', 'Kategori', 'text', true),
                        $this->field('severity', 'Tingkat', 'select', true, CleaningFindingSeverity::class),
                        $this->field('status', 'Status', 'select', true, CleaningFindingStatus::class),
                        $this->field('description', 'Deskripsi', 'textarea', true),
                        $this->field('corrective_action', 'Tindakan sementara/koreksi', 'textarea'),
                        $this->field('due_at', 'Batas tindak lanjut', 'datetime'),
                        $this->field('resolved_at', 'Waktu selesai', 'datetime'),
                        $this->field('resolved_by', 'Diselesaikan oleh', 'select', false, 'users'),
                        $this->field('photo_path', 'Foto', 'file'),
                        $this->field('notes', 'Catatan', 'textarea'),
                    ]),
                ],
            ],
